...
func buildCodexCliPassHeaderTemplate(headers []string) map[string]interface{} {
	clonedHeaders := make([]string, 0, len(headers))
	clonedHeaders = append(clonedHeaders, headers...)
	return map[string]interface{}{
		"operations": []map[string]interface{}{
	...
	Rules: []ChannelAffinityRule{
		{
			...
			ValueRegex:            "",
			TTLSeconds:            0,
			ParamOverrideTemplate: buildCodexCliPassHeaderTemplate(codexCliPassThroughHeaders),
